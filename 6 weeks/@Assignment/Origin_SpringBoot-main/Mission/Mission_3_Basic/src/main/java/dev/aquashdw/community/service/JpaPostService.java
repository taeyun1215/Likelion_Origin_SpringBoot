...
public class JpaPostService implements PostService{
    ...
    public PostDto create(Long boardId, PostDto postDto) {
        ...
        postEntity.setWriter(userEntity);   // ManyToOne 으로 이어져있음
        postEntity.setBoardEntity(boardEntity); // ManyToOne 으로 이어져있음
        postEntity = this.postRepository.save(postEntity);  // 저장은 Repository 에 해주기

        return new PostDto( // 반환은 DTO로 해주기
                postEntity.getId(),
                postEntity.getTitle(),
    ...
    public boolean update(Long boardId, Long postId, PostDto dto) {
        ...
        if (!postEntity.getBoardEntity().getId().equals(boardId))   // 직접 입력한 boardId와 postEntity 에 있는 boardId가 같은지 확인
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST);
